# Rari DAI Pool: Smart Contract API

Welcome to the API docs for `RariFundManager`, `RariFundToken`, and `RariFundProxy`, the smart contracts behind the Rari DAI Pool.
...
Returns the total balance in USD (scaled by 1e18) supplied to the Rari DAI Pool by `account`.
...
Deposits funds to the Rari DAI Pool in exchange for RDPT (with GSN support).
...
Deposits funds to the Rari DAI Pool in exchange for RDPT.
...
Exchanges and deposits funds to the Rari DAI Pool in exchange for RDPT (via 0x).
...
Exchanges and deposits funds to the Rari DAI Pool in exchange for RDPT (no slippage and low fees via mStable, but only supports DAI, USDC, USDT, TUSD, and mUSD).
...
Withdraws funds from the Rari DAI Pool in exchange for RDPT.
...
* Please note that you must approve RariFundManager to burn of the necessary amount of RDPT.
* Parameters:
...
Withdraws funds from the Rari DAI Pool in exchange for RDPT and exchanges to them to the desired currency (if no 0x orders are supplied, exchanges DAI, USDC, USDT, TUSD, and mUSD via mStable).

* You can retrieve order data from the [0x swap API](https://0x.org/docs/api#get-swapv0quote). See [`USAGE.md`](USAGE.md), the SDK, or the web client for implementation.
* Please note that you must approve RariFundManager to burn of the necessary amount of RDPT. You also must input at least enough ETH to cover the protocol fees.
* Parameters:
...
See [EIP-20: ERC-20 Token Standard](https://eips.ethereum.org/EIPS/eip-20) for reference on all common functions of ERC20 tokens like RDPT. Here are a few of the most common ones:

### `uint256 RariFundToken.balanceOf(address account)`

Returns the amount of RDPT owned by `account`.

* A user's RDPT balance is an internal representation of their USD balance.
    * While a user's USD balance is constantly increasing as the Rari DAI Pool accrues interest, a user's RDPT balance does not change except on deposit, withdrawal, and transfer.
    * The price of RDPT is equivalent to the current value of the first $1 USD deposited to the Rari DAI Pool.
* Parameters:
    * `account` (address) - The account whose balance we are retrieving.

### `bool RariFundToken.transfer(address recipient, uint256 amount)`

Transfers the specified `amount` of RDPT to `recipient`.

* Parameters:
    * `recipient` (address): The recipient of the RDPT.
    * `inputAmounts` (uint256[]): The amounts of tokens to be withdrawn and exchanged (including taker fees).
* Return value: Boolean indicating success.

### `bool RariFundToken.approve(address spender, uint256 amount)`

Approve `sender` to spend the specified `amount` of RDPT on behalf of `msg.sender`.
...
Returns the total supply of RDPT (scaled by 1e18).

* Divide `RariFundManager.getFundBalance()` by `RariFundToken.totalSupply()` to get the exchange rate of RDPT in USD (scaled by 1e18).

## **Total Supply and Interest**

### `uint256 RariFundManager.getFundBalance()`

Returns the total balance supplied by users to the Rari DAI Pool (all RDPT holders' funds but not unclaimed fees) in USD (scaled by 1e18).
...
Returns the total amount of interest accrued (excluding the fees paid on interest) by past and current Rari DAI Pool users (i.e., RDPT holders) in USD (scaled by 1e18).
...
Returns the raw total balance of the Rari DAI Pool (all RDPT holders' funds + all unclaimed fees) of all currencies in USD (scaled by 1e18).
...
Returns the raw total balance of the Rari DAI Pool (all RDPT holders' funds + all unclaimed fees) of the specified currency.
...
Returns the raw total amount of interest accrued by the Rari DAI Pool (including the fees paid on interest) in USD (scaled by 1e18).
